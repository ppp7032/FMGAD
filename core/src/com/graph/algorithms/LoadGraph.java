...
public class LoadGraph implements Screen {
  ...
  public LoadGraph(final Graph graph, final ArrayList<EdgeWeight> edgeWeights,
      final ArrayList<Text> vertexLabels) {
    this.graph = graph;
    alertMessage = new Text("", Gdx.graphics.getWidth() / 2f, Gdx.graphics.getHeight() / 2f,
        Graphics.fonts[3], new float[]{0, 0, 0, 1}, 0, 0, -1);
    ...
    final TextButton routeInspectionButton = new TextButton("C. Postman", Graphics.skins[1],
        "default");
    final TextButton travellingSalesmanButton = new TextButton("T. Salesman", Graphics.skins[1],
        "default");
    ...
    sidePanelButtons = new TextButton[]{mainMenu, edit, dijkstraButton, jarnikButton, kruskalButton,
        routeInspectionButton, travellingSalesmanButton, help};
    final Text menuTitle = new Text("", Gdx.graphics.getWidth() / 2f, 545 * Graphics.scaleFactor,
        Graphics.fonts[4], new float[]{0, 0, 0, 1}, 0, 0, -1);
    final Text[] attributes = getAttributes();
    ...
    routeInspectionItems.add(new ArrayList<>());
    final ArrayList<ArrayList<String>> lowerBoundTSPItems = new ArrayList<>();
    lowerBoundTSPItems.add(new ArrayList<>());
    ...
    dijkstraButton.setPosition(80f * Graphics.scaleFactor - dijkstraButton.getWidth() / 2f,
        652f * Graphics.scaleFactor);
        ...
        if (!graph.isDigraph()) {
          ...
          if (!everRanRouteInspection) {
            ...
            if (repeatedEdges != null) {
              ...
              items[0] = "Total Weight: " + repeatedEdges.getLast().getFirst();
              for (int a = 1; a < items.length; a++) {
                final ArrayList<Integer> numbers = repeatedEdges.get(a - 1);
                final char[] numbersChar = {(char) (numbers.get(0) + 65),
                    (char) (numbers.get(1) + 65)};
            ...
            list.setItems(routeInspectionItems.getFirst().toArray(new String[0]));
          }
        } else {
          enableDigraphAlert();
        }
          ...
          if (selectTSPAlgorithm.getSelectedIndex() == 1) {
            if (!graph.isDigraph()) {
              ...
              if (!everRanLowerBounds) {
                ...
                list.setItems(lowerBoundTSPItems.getFirst().toArray(new String[0]));
              }
            } else {
              enableDigraphAlert();
            }
  ...
  private static Text[] getAttributes() {
    final float y1 = 491.5f;
    return new Text[]{
        new Text("", 16f * Graphics.scaleFactor + (0.5f * (Gdx.graphics.getWidth()
            - 452 * Graphics.scaleFactor)), y1 * Graphics.scaleFactor, Graphics.fonts[3],
            new float[]{0, 0, 0, 1}, -1, 0, -1),
        new Text("", 16f * Graphics.scaleFactor + (0.5f * (Gdx.graphics.getWidth()
            - 452 * Graphics.scaleFactor)), (y1 - 61f) * Graphics.scaleFactor,
            Graphics.fonts[3], new float[]{0, 0, 0, 1}, -1, 0, -1)
    };
  }
  ...
  private void setUpDijkstraBoxes() {
    ...
    for (int a = 0; a < graph.getNumberOfVertices(); a++) {
      ...
      dijkstraLabels[a] = new Text[]{
          new Text(Character.toString((char) (a + 65)), dimensions[0] + dimensions[2] / 6f,
              dimensions[1] + dimensions[3] / 4f * 3f, Graphics.fonts[2], new float[]{0, 0, 0, 1},
              0, 0, 31 * Graphics.scaleFactor),
          new Text("", dimensions[0] + dimensions[2] / 2f, dimensions[1] + dimensions[3] / 4f * 3f,
              Graphics.fonts[2], new float[]{0, 0, 0, 1}, 0, 0, 31 * Graphics.scaleFactor),
          new Text("", dimensions[0] + dimensions[2] / 6f * 5f,
              dimensions[1] + dimensions[3] / 4f * 3f, Graphics.fonts[2], new float[]{0, 0, 0, 1},
              0, 0, 31 * Graphics.scaleFactor),
          new Text("", dimensions[0] + 5f * Graphics.scaleFactor,
              dimensions[1] + dimensions[3] / 4f, Graphics.fonts[0], new float[]{0, 0, 0, 1}, -1, 0,
              dimensions[2] - 10f * Graphics.scaleFactor)
      };
    }
  }
  ...
  private void toggleDijkstraPressed(final Text menuTitle, final Text startVertexLabel,
      final Text endVertexLabel, final TextButton back, final TextButton apply) {
    final boolean newStatus = !menuTitle.isVisible();
    if (newStatus) {
  ...
  private void toggleJarnikPressed(final Text menuTitle, final Text startVertexLabel,
      final TextButton back, final TextButton apply) {
    final boolean newStatus = !menuTitle.isVisible();
    if (newStatus) {
  ...
  private void toggleChinesePostman(final Text menuTitle, final ScrollPane scrollBar,
  ...
  private void toggleTravellingSalesman(final Text menuTitle, final Text algorithmLabel,
      final SelectBox<String> selector, final TextButton back, final TextButton apply) {
    final boolean newStatus = !menuTitle.isVisible();
    if (newStatus) {
  ...
  private void toggleLowerBounds(final Text menuTitle, final ScrollPane scrollBar,
      final TextButton back) {
    final boolean newStatus = !menuTitle.isVisible();
    if (newStatus) {
  ...
  private void toggleNearestNeighbourPressed(final Text menuTitle, final Text startVertexLabel,
      final TextButton back, final TextButton apply) {
    final boolean newStatus = !menuTitle.isVisible();
    if (newStatus) {
  ...
  private void drawMST(boolean condition) {
    if (condition) {
      for (int a = 0; a < counter; a++) {
        Graphics.renderEdge(graph.getXCoordinateOfVertex(minimumEdges.get(a)[0]),
            graph.getYCoordinateOfVertex(minimumEdges.get(a)[0]),
            graph.getXCoordinateOfVertex(minimumEdges.get(a)[1]),
            graph.getYCoordinateOfVertex(minimumEdges.get(a)[1]), shapeRenderer, false);
      }
      if (Gdx.input.isKeyJustPressed(Input.Keys.SPACE)
          && counter < graph.getNumberOfVertices() - 1) {
        counter++;
      }
    }
  }
  ...
  public void render(float delta) {
    ...
    if (dijkstraApplied && dijkstraContainer.getSetup()) {
      ArrayList<Integer> path = dijkstraContainer.getPathToVertex(
          dijkstraContainer.getLargestOrderLabel());
      shapeRenderer.setColor(0, 1, 0, 1);
      for (int a = 1; a < path.size(); a++) {
        final int vertex1 = path.get(a - 1);
        final int vertex2 = path.get(a);
        Graphics.renderEdge(graph.getXCoordinateOfVertex(vertex1),
            graph.getYCoordinateOfVertex(vertex1), graph.getXCoordinateOfVertex(vertex2),
            graph.getYCoordinateOfVertex(vertex2), shapeRenderer, graph.isDigraph());
      }
    } else if (nearestNeighbourApplied) {
      if (counter == nearestNeighbourPath.size() - 2
          && nearestNeighbourPath.getLast() == null) {
        alertMessage.updateText("Nearest Neighbour\nhas stalled!");
        alertMessage.setVisible(true);
      } else if (counter < nearestNeighbourPath.size() - 2 && Gdx.input.isKeyJustPressed(
          Input.Keys.SPACE)) {
        counter++;
      }
      shapeRenderer.setColor(0, 1, 0, 1);
    ...
    if (dijkstraPressed) {
      ...
      for (int a = 0; a < graph.getNumberOfVertices(); a++) {
        ...
        Graphics.drawRectangleWithBorder(shapeRenderer, dimensions[0], dimensions[1], dimensions[2],
            dimensions[3], 2 * Graphics.scaleFactor, new float[]{1, 1, 1, 1});
        shapeRenderer.rectLine(dimensions[0], dimensions[1] + dimensions[3] / 2f,
            dimensions[0] + dimensions[2], dimensions[1] + dimensions[3] / 2f,
            2 * Graphics.scaleFactor);
        shapeRenderer.rectLine(dimensions[0] + dimensions[2] / 3f, dimensions[1] + dimensions[3],
            dimensions[0] + dimensions[2] / 3f, dimensions[1] + dimensions[3] / 2f,
            2f * Graphics.scaleFactor);
        shapeRenderer.rectLine(dimensions[0] + dimensions[2] / 3f * 2f,
            dimensions[1] + dimensions[3] / 2f, dimensions[0] + dimensions[2] / 3f * 2f,
            dimensions[1] + dimensions[3], 2 * Graphics.scaleFactor);
      }
      if (!dijkstraContainer.getSetup()) {
        dijkstraContainer = new DijkstraContainer(
            graph.getVertexFromInput(startVertexInput.getText()),
            graph.getVertexFromInput(endVertexInput.getText()), graph.getNumberOfVertices());
        graph.updateDijkstraLabels(dijkstraLabels, dijkstraContainer.getOrderLabels(),
            dijkstraContainer.getPermanentLabels(), dijkstraContainer.getTemporaryLabels());
      } else if (Gdx.input.isKeyJustPressed(Input.Keys.SPACE)
          && dijkstraContainer.getOrderLabels()[dijkstraContainer.getEndVertex()] == -1) {
    ...
    } else if (travellingSalesmanPressed || nearestNeighbourPressed || jarnikPressed
        || viewingHelp) {
    ...
    Graphics.drawRectangleWithBorder(shapeRenderer, Graphics.scaleFactor, 0,
        160f * Graphics.scaleFactor, Gdx.graphics.getHeight() - Graphics.scaleFactor,
        2f * Graphics.scaleFactor, new float[]{207f / 255f, 226f / 255f, 243f / 255f, 1});
    shapeRenderer.end();
    stage.act();
    stage.draw();
    if (dijkstraApplied) {
