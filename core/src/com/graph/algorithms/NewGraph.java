...
public class NewGraph implements Screen {
  ...
  public NewGraph(final Graph graph, final ArrayList<EdgeWeight> edgeWeights,
    ...
    temporaryVertexLabel = new Text(Character.toString((char) (graph.getNumberOfVertices() + 65)),
        0, 0, Graphics.fonts[3], new float[]{0, 0, 0, 1}, 0, 0, -1);
    ...
    final TextButton[] sidePanelButtons = new TextButton[]{newVertex, newEdge, save, help, mainMenu,
        finish};
    ...
    name.setPosition(80f * Graphics.scaleFactor - name.getWidth() / 2f,
    ...
    newVertex.setPosition(80f * Graphics.scaleFactor - newVertex.getWidth() / 2f,
        name.getY() - 71 * Graphics.scaleFactor);
      ...
      public void clicked(InputEvent event, float x, float y) {
        if (mouseInBoundsForVertex()) {
          if (newVertexClicked) {
            ...
            vertexLabels.add(
                new Text(Character.toString((char) (graph.getNumberOfVertices() + 64)), x, y,
                    Graphics.fonts[3], new float[]{0, 0, 0, 1}, 0, 0, -1));
            ...
            if (clickedVertex != -1) {
              ...
              } else if (firstVertex != clickedVertex && !graph.areTwoVerticesConnected(firstVertex,
                  clickedVertex)) {
    ...
    back.addListener(new ClickListener() {
      ...
      public void clicked(InputEvent event, float x, float y) {
        if (newEdgeClicked) {
          closeEnterEdgeWeightMenu(edgeWeight, menuTitle, attributes[0], back, apply,
              sidePanelButtons, name);
          stage.setKeyboardFocus(null);
        } else if (viewingHelp) {
    ...
    apply.addListener(new ClickListener() {
      ...
      public void clicked(InputEvent event, float x, float y) {
        ...
        if (isInteger) {
          ...
          edgeWeights.add(new EdgeWeight(graph, firstVertex, secondVertex, Integer.toString(weight),
              Graphics.fonts[3], new float[]{0, 0, 1, 1}, 0, 0));
          closeEnterEdgeWeightMenu(edgeWeight, menuTitle, attributes[0], back, apply,
              sidePanelButtons, name);
        }
      }
  ...
  private static Text[] getAttributes() {
    ...
    return new Text[]{
        new Text("", 16 * Graphics.scaleFactor + (0.5f * (Gdx.graphics.getWidth()
            - 452 * Graphics.scaleFactor)), y1 * Graphics.scaleFactor, Graphics.fonts[3],
            new float[]{0, 0, 0, 1}, -1, 0, -1),
        new Text("Press 'E' to add a new edge",
            16 * Graphics.scaleFactor + (0.5f * (Gdx.graphics.getWidth()
                - 452 * Graphics.scaleFactor)), (y1 - 61f) * Graphics.scaleFactor,
            Graphics.fonts[3], new float[]{0, 0, 0, 1}, -1, 0, -1),
        new Text("Right click a vertex to delete it",
            16 * Graphics.scaleFactor + (0.5f * (Gdx.graphics.getWidth()
                - 452 * Graphics.scaleFactor)), (y1 - 61f * 3) * Graphics.scaleFactor,
            Graphics.fonts[3], new float[]{0, 0, 0, 1}, -1, 0, -1),
        new Text("Drag a vertex to move it",
            16 * Graphics.scaleFactor + (0.5f * (Gdx.graphics.getWidth()
                - 452 * Graphics.scaleFactor)), (y1 - 61f * 2) * Graphics.scaleFactor,
            Graphics.fonts[3], new float[]{0, 0, 0, 1}, -1, 0, -1),
        new Text("Only integer weights are supported",
            16 * Graphics.scaleFactor + (0.5f * (Gdx.graphics.getWidth()
                - 452 * Graphics.scaleFactor)), (y1 - 61f * 4) * Graphics.scaleFactor,
            Graphics.fonts[3], new float[]{0, 0, 0, 1}, -1, 0, -1)
    };
  }
  ...
  private void toggleHelpMenu(final boolean newStatus, final TextButton[] sidePanelButtons,
      final TextButton back, final Text menuTitle, final Text[] attributes) {
    ...
    if (!newEdgeClicked && !alertMessage.isVisible()) {
      if (!newVertexClicked) {
        if (graph.getNumberOfVertices() < 26) {
          temporaryVertexLabel.updateText(
              Character.toString((char) (graph.getNumberOfVertices() + 65)));
  ...
  private boolean mouseInBoundsForVertex() {
    return Gdx.input.getX() / Graphics.scaleFactor - 15 > 160f
        && Gdx.input.getY() - 15 * Graphics.scaleFactor > 0
        && Gdx.input.getY() + 15 * Graphics.scaleFactor < Gdx.graphics.getHeight()
        && Gdx.input.getX() + 15 * Graphics.scaleFactor < Gdx.graphics.getWidth();
  }
  ...
  private int findVertexBeingClicked() {
    for (int a = 0; a < graph.getNumberOfVertices(); a++) {
      ...
      if (Math.pow(mouseX - graph.getXCoordinateOfVertex(a), 2) + Math.pow(
          mouseY - graph.getYCoordinateOfVertex(a), 2) <= 15 * 15) {
        return a;
      }
    }
    return -1;
  ...
  private void closeEnterEdgeWeightMenu(final TextField edgeWeight, final Text edgeWeightTitle,
      final Text edgeWeightLabel, final TextButton back, final TextButton apply,
      final TextButton[] sidePanelButtons, final TextField name) {
  ...
  public void render(final float delta) {
    ...
    if (Gdx.input.isButtonPressed(Input.Keys.LEFT)) {
      if (vertexBeingMoved == -1 && !newEdgeClicked && !newVertexClicked
          && Graphics.mouseInBounds()) {
    ...
    if (vertexBeingMoved != -1 && mouseInBoundsForVertex()) {
      graph.setCoordinates(vertexBeingMoved, new float[]{Gdx.input.getX() / Graphics.scaleFactor,
          (Gdx.graphics.getHeight() - Gdx.input.getY()) / Graphics.scaleFactor});
    }
    if (stage.getKeyboardFocus() != name) {
      if (Gdx.input.isKeyJustPressed(Input.Keys.V)) {
        ...
        clickNewEdge();
      } else if (Gdx.input.isButtonJustPressed(Input.Buttons.RIGHT) && Graphics.mouseInBounds()
          && !newVertexClicked && !newEdgeClicked && !alertMessage.isVisible()) {
    ...
    if (newEdgeClicked && firstVertex != -1) {
      if (secondVertex == -1) {
        if (Graphics.mouseInBounds()) {
          Graphics.renderEdge(graph.getXCoordinateOfVertex(firstVertex),
              graph.getYCoordinateOfVertex(firstVertex), Gdx.input.getX() / Graphics.scaleFactor,
              (Gdx.graphics.getHeight() - Gdx.input.getY()) / Graphics.scaleFactor, shapeRenderer,
              graph.isDigraph());
        }
      } else {
        Graphics.renderEdge(graph.getXCoordinateOfVertex(firstVertex),
            graph.getYCoordinateOfVertex(firstVertex), graph.getXCoordinateOfVertex(secondVertex),
            graph.getYCoordinateOfVertex(secondVertex), shapeRenderer, graph.isDigraph());
      }
    }
    Graphics.renderGraphVertices(shapeRenderer, graph, vertexLabels, stage.getBatch(),
        vertexBeingMoved);
    shapeRenderer.end();
    if (newVertexClicked && mouseInBoundsForVertex()) {
      shapeRenderer.begin(ShapeRenderer.ShapeType.Filled);
      Graphics.renderVertex(shapeRenderer, stage.getBatch(),
          Gdx.input.getX() / Graphics.scaleFactor,
          (Gdx.graphics.getHeight() - Gdx.input.getY()) / Graphics.scaleFactor,
          temporaryVertexLabel);
      shapeRenderer.end();
    }
