...
public abstract class Graphics {
    ...
    public static void drawSelectionMenu(final SpriteBatch spriteBatch, final Texture background, final ShapeRenderer shapeRenderer, final Stage stage, final int numberOfAttributes) {
    ...
    public static void renderGraphEdges(final ShapeRenderer shapeRenderer, final Graph graph) {
        ...
        for (int a = 0; a < graph.getNumberOfVertices(); a++) {
            for (int b = 0; b < graph.getNumberOfEdgesConnectedToVertex(a); b++) {
                Graphics.renderEdge(graph.getXCoordinateOfVertex(a), graph.getYCoordinateOfVertex(a), graph.getXCoordinateOfVertex(graph.getVertex(a, b)), graph.getYCoordinateOfVertex(graph.getVertex(a, b)), shapeRenderer, graph.isDigraph());
            }
        }
    }
    ...
    private static void renderVertex(final ShapeRenderer shapeRenderer, final Batch batch, final Graph graph, final int vertex, final Text vertexLabel) {
        renderVertex(shapeRenderer, batch, graph.getXCoordinateOfVertex(vertex), graph.getYCoordinateOfVertex(vertex), vertexLabel);
    }

    public static void renderVertex(final ShapeRenderer shapeRenderer, final Batch batch, final float x, final float y, final Text vertexLabel) {
        shapeRenderer.circle(x * Graphics.scaleFactor, y * Graphics.scaleFactor, 15 * Graphics.scaleFactor);
        shapeRenderer.setColor(247f / 255f, 247f / 255f, 247f / 255f, 1);
        shapeRenderer.circle(x * Graphics.scaleFactor, y * Graphics.scaleFactor, 13 * Graphics.scaleFactor);
        shapeRenderer.end();
        batch.begin();
    ...
    public static void renderGraphVertices(final ShapeRenderer shapeRenderer, final Graph graph, final ArrayList<Text> vertexLabels, Batch batch, final int vertexBeingMoved) {
        shapeRenderer.setColor(0, 0, 0, 1);
        ...
        if (vertexBeingMoved != -1) {
            renderVertex(shapeRenderer, batch, graph, vertexBeingMoved, vertexLabels.get(vertexBeingMoved));
        }
    }
    ...
    public static void renderEdge(final float x1, final float y1, final float x2, final float y2, final ShapeRenderer shapeRenderer, final Boolean digraphStatus) {
        shapeRenderer.rectLine(x1 * Graphics.scaleFactor, y1 * Graphics.scaleFactor, x2 * Graphics.scaleFactor, y2 * Graphics.scaleFactor, 5 * Graphics.scaleFactor);
        if (digraphStatus) {
            final float[][] points = Graphics.arrowHeadGenerator(new float[]{x1, y1}, new float[]{x2, y2});
            shapeRenderer.triangle(points[0][0], points[0][1], points[1][0], points[1][1], points[2][0], points[2][1]);
        }
    }

    public static void addTextToMenu(final Stage stage, final String title, final String[] attributes, final BitmapFont titleFont, final BitmapFont attributeFont) {
        final float[] black = new float[]{0, 0, 0, 1};
        stage.addActor(new Text(title, Gdx.graphics.getWidth() / 2f, 545 * Graphics.scaleFactor, titleFont, black, 0, 0, -1));
        float y = 491.5f;
        for (String attribute : attributes) {
            stage.addActor(new Text(attribute, 0.5f * (Gdx.graphics.getWidth() - 452 * Graphics.scaleFactor) + 16f * Graphics.scaleFactor, y * Graphics.scaleFactor, attributeFont, black, -1, 0, -1));
            y -= 61f;
        }
    }
    ...
    public static void drawMenu(final int numberOfAttributes, final ShapeRenderer shapeRenderer) {
        ...
        Graphics.drawRectangleWithBorder(shapeRenderer, x, y, width, height, 2 * Graphics.scaleFactor, blue);
        if (numberOfAttributes > 0) {
            ...
            Graphics.drawRectangleWithBorder(shapeRenderer, x, y, width, height, 2 * Graphics.scaleFactor, yellow);
    ...
    public static void setupBackAndApplyButtons(final TextButton back, final TextButton apply, final boolean visible) {
    ...
    public static boolean mouseInBounds() {
        return Gdx.input.getX() / Graphics.scaleFactor > 160f && Gdx.input.getY() > 0 && Gdx.input.getY() < Gdx.graphics.getHeight() && Gdx.input.getX() < Gdx.graphics.getWidth();
    }
    ...
    public static void setupBottomButton(final TextButton button) {
        ...
        button.setPosition(80f * Graphics.scaleFactor - button.getWidth() / 2f, 24f * Graphics.scaleFactor);
    }
